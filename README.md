...
### Recruiter Portal
- **Recruiter Registration & Company Profile**: Register with recruiter full name, official email, phone, corporate designation, and company profile. New registrations start in **`Pending Approval`** (`PENDING`) status with company verification pending (`is_verified = 0`).
- **Verification Guard & Dashboard Alert**: While pending admin review, the recruiter dashboard clearly alerts: *"Your recruiter account is waiting for admin approval."* Job creation controls are locked while company profile viewing and updates remain accessible.
- **Direct Job Publication Engine (Post-Approval)**: Once approved by an administrator, recruiters can publish vacancies directly into the live catalog (`LIVE` status) without requiring per-job micro-approvals. Postings specify employment type (Full-Time, Internship, Part-Time), work mode (On-Site, Hybrid, Remote), location, stipend/salary, minimum CGPA cutoff, required technical competencies, and eligible branches.
- **Job Listings Management**: Monitor, edit, or close active listings when vacancies are filled.
...
- **Recruiter Alerts**: Receive notifications when students submit applications.
...
- **Dedicated Recruiter Verification Queue**: Dedicated **Pending Recruiters (Awaiting Approval)** section displaying recruiter name, email, designation, company name, industry, location, website, and registration date with 1-click **Approve** and **Reject** controls.
- **Recruiter Status Filtering & Permission Control**: Filter recruiters across **All**, **Pending**, **Approved**, and **Rejected** tabs with live count badges, with the ability to revoke access from active recruiters or re-approve previously rejected accounts.
- **Student Directory**: Search, inspect, and filter the complete student database by branch, academic year, and placement status.
- **Job Drive Management**: Central registry to monitor and review all campus job postings across companies, ensuring only verified employers are active.
...
└──────┬──────┘
       │
       ▼
   Registers Account & Company Profile (Status: 'PENDING', Company Unverified)
       │
       ▼
┌─────────────┐
│    Admin    │
└──────┬──────┘
       │
       ▼
   Reviews in "Pending Recruiters" Queue & Approves (Status: 'APPROVED', Company Verified)
       │
       ▼
┌─────────────┐
│  Recruiter  │
└──────┬──────┘
       │
       ▼
   Posts Job Directly (Title, Salary, CGPA Cutoff, Skills, Branches)
       │
       ▼
   Saved in Database (`jobs`, status: 'LIVE')
       │
       ├────────────────────────────────────────┐
       ▼                                        ▼
┌─────────────┐                          ┌─────────────┐
...
└──────┬──────┘                          └─────────────┘
       │                                        │
       ▼                                        ▼
   Browses Verified Opportunities         Monitors Campus Job Drive
       │
       ▼
...
11. **`recruiters`**: Recruiter profiles linked to user accounts and companies, capturing full name (`recruiter_name`), corporate designation, phone, and verification lifecycle status (`approval_status`: `PENDING`, `APPROVED`, `REJECTED`).
12. **`jobs`**: Job postings with employment type, work mode, salary/stipend, CGPA cutoff, deadline, and status (`LIVE`, `PENDING`, `CLOSED`). Jobs from approved recruiters and verified companies are published live to eligible students.
...
- **5 Triggers**: Enforces business rules (preventing applications on closed jobs, updating student placement status upon selection, audit logging).
...
| :--- | :---: | :---: | :---: |
| **Register & Sign In** | Yes | Yes | Yes (Pre-configured) |
| **Manage Profile, Skills & Resume** | Yes | Company Profile | System Profile |
| **Browse Job Openings** | Yes (Verified Jobs) | Yes (Own Jobs) | Yes (All Jobs) |
...
| **Publish & Manage Job Postings** | No | Yes (When Approved) | Yes |
| **Verify Recruiters & Companies** | No | No | Yes |
...
#### 2. Recruiter
- **Account Creation:** Corporate recruiters register with their full name, corporate email, phone, corporate designation, and company profile. Accounts initialize with **`Pending Approval`** (`PENDING`) status and an unverified company profile.
- **Key Responsibilities:** Set up and maintain company profile; await T&P administrator verification (dashboard notifies pending status and locks posting actions); publish campus vacancies directly upon approval (`LIVE` status without requiring per-job micro-approvals); review and filter candidate applications; inspect applicant skills, projects, and PDF resumes; shortlist qualified candidates; schedule multi-round interviews with physical venues or video meeting links; submit interview evaluations (`PASSED`/`FAILED`); and extend official placement offers via atomic database transaction (`sp_process_candidate_selection`).
...
- **Key Responsibilities:** Review new recruiter and company registrations in the dedicated Pending Recruiters queue; verify corporate credentials and approve or reject recruiters; revoke access or re-authorize recruiters as needed; oversee institution-wide campus hiring drives; inspect all student applications, interview calendars, and placement records; manage academic departments, branches, and the technical skills repository; and generate official placement statistics for institutional accreditation (NAAC/NIRF).
...
- **Admin Portal**: Institutional Analytics Dashboard, Recruiter Approvals, Placement Records
...
5. **Recruiter Verification Guard**: Server-side authorization (`requireApprovedRecruiter()`) strictly blocks unapproved (`PENDING`) or rejected (`REJECTED`) recruiter accounts from publishing jobs via direct API calls (HTTP 403 Forbidden), guaranteeing that verification is enforced at the backend architecture level and not solely dependent on UI controls.
